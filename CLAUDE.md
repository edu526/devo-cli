...
- **macOS PATH (fix in progress, unconfirmed)**: apps launched from Finder/Dock get LaunchServices' minimal PATH (no Homebrew, no shell rc PATH additions), which the sidecar inherits — so bare `subprocess.run(["aws", ...])` calls fail with `FileNotFoundError` even though `aws` is installed, while profile listing (which reads `~/.aws/config` directly, no subprocess) works fine. First attempt (`sidecar.rs`'s `sidecar_env_overrides()` resolving `$SHELL -ilc 'echo $PATH'` and passing it as an env override) shipped in `app-v0.1.0-alpha.13` but a real mac user still hit the error — the shell-resolution likely silently failed/timed out and no override was applied at all. Hardened it: `known_macos_path_dirs()` now unconditionally adds `/opt/homebrew/bin`, `/usr/local/bin`, etc. (the two dominant `aws` install methods) regardless of whether shell resolution succeeds, and `append_log_line()` writes the outcome ("resolved login-shell PATH" vs "falling back to known dirs only") to `~/.devo/sidecar.log` as a `WARN [macos-path-fix]` line so the next report shows what actually happened, without needing terminal access. Still needs confirmation from a real mac run.
- **Linux binary name collision (fixed)**: the Cargo package is named `devo` (`desktop/src-tauri/Cargo.toml`), same as the CLI's installed command (`pyproject.toml`'s `[project.scripts] devo = ...`). On Debian/Ubuntu, `/usr/local/bin` and `~/.local/bin` (where the CLI installs) precede `/usr/bin` (where the `.deb` installs the desktop binary) in PATH — including in graphical sessions, since `/etc/environment` sets that order too — so whichever the CLI install picked could silently shadow the desktop app's binary for anyone launching it via a `.desktop` Exec without an absolute path. Fixed by setting `"mainBinaryName": "devo-desktop"` in `tauri.conf.json`, which only renames the binary produced by `tauri build`/`tauri bundle` (dev builds via `cargo build`/`pnpm tauri dev` are unaffected and still produce a raw `devo`/`devo.exe`, so `kill-zombies.sh`/`.ps1` correctly still target `devo`). Windows/macOS were never affected — desktop app directories there aren't on PATH by default.

- **Linux WebKitGTK EGL abort (fixed)**: on Ubuntu 22.04+ with WebKitGTK ≥ 2.42 (NVIDIA proprietary drivers, VMs, some X11 setups) the app died at launch with `Could not create GBM EGL display: EGL_NOT_INITIALIZED. Aborting...` (SIGABRT). `main.rs` now sets `WEBKIT_DISABLE_DMABUF_RENDERER=1` on Linux before Tauri starts, unless the user already set it.
