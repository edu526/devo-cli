...
def _get_shell_config(shell_name: str, profile_name: str) -> tuple:
    ...
    home = Path.home()
    cfg = _SHELL_CONFIG.get(shell_name, _SHELL_CONFIG["bash"])
    config_file = home / cfg["file"]
    export_line = cfg["export"].format(profile=profile_name)
    return config_file, export_line


def _update_shell_config_file(config_file: Path, export_line: str, profile_name: str):
...
def _set_unix_profile(profile_name: str):
    ...
    _update_shell_config_file(config_file, export_line, profile_name)
    console.print(f"\n[green]✓ Set '{profile_name}' as default profile[/green]")
