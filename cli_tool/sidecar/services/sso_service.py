...
def run_sso_login_sync(hub: EventHub, profile_name: str, source: str) -> bool:
    ...
    try:
        ...
        process = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1, env=env, shell=platform.system() == "Windows"
        )
        ...
        if process.returncode == 0:
            ...
            # Retry verify_credentials up to 5 times with a 2-second delay between attempts.
            import time

            verified = None
            for attempt in range(5):
                verified = verify_credentials(profile_name)
                if verified:
                    break
                if attempt < 4:
                    logger.debug("verify_credentials attempt %d failed for '%s', retrying...", attempt + 1, profile_name)
                    time.sleep(2)
            ...
            if verified:
                ...
                msg = f"Credential verification failed for '{profile_name}' after SSO login. Try restarting the connection."
                logger.error(msg)
